...
def train_super_learner(x_train, x_test, y_train, y_test, param_grid, cv):
    #Define three classification models as base classifiers consisting of Naïve Bayes, Neural Network, and KNN
    nb_model = GaussianNB()
    knn_model = KNeighborsClassifier()
    nn_model = MLPClassifier(max_iter=200, random_state=0)
    ...
    return best_params, best_train_accuracy,test_accuracy
...
best_params, best_train_accuracy,test_accuracy = train_super_learner(train_pca, test_pca, train_labels, test_labels,cv=5,param_grid=param_grid)
print(f"Train Accuracy on Diabetes Dataset: {best_params}")
print(f"Train Accuracy on Diabetes Dataset: {best_train_accuracy}")
print(f"Test Accuracy on Diabetes Dataset: {test_accuracy}")
...
best_params_cancer, best_train_accuracy_cancer, accuracy_breast_cancer = train_super_learner(train_pca_df, test_pca_df, label_train, label_test,cv=3,param_grid=param_grid)
print(f"Best_params on Breast_Cancer_Survival Dataset: {best_params}")
print(f"Train Accuracy on Breast_Cancer_Survival Dataset: {best_train_accuracy}")
print(f"Test Accuracy on Breast_Cancer_Survival Dataset: {accuracy_breast_cancer}")
